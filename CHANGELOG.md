...
that moment no re-run in any mode refreshes a drifted `commands/polish.md` again — see the F-10 entry
below for the mechanism and the affected messages. The three deny messages F-10 fixes now describe this
root cause accurately, but do not fix it (dir #323). Second, a real-file (non-symlink) `bin/keel` is
left untouched by `install.sh`, whose refusal message says "remove it to let Keel wire the CLI." Both
`bin/keel`'s own severed-checkout advice and `doctor.sh`'s W-CLI-UNWIRED warning instead tell the reader
to re-run `install.sh`, so the one correct remedy is reachable only by reading `install.sh`'s own
refusal message (dir #324).
