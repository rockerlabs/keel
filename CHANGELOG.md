...
- **An inherited `GIT_DIR`/`GIT_COMMON_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE` in the environment can no
  longer redirect a `git -C` call into a different repository than the one actually named** (dir #644,
  closing dir #318's residual N8): `tests/lib.sh` unsets all four before its own first git call, so
  `-C` is the only thing left that can select a repo for a test process and everything it spawns —
  `tests/test_lib_ref_guard.sh` gains T13, reproducing the escape live and pinning the closure. New
  `tools/lib/repo-arg-guard.sh` does the same, unconditionally, at source time, for every OTHER keel
  tool that calls `git -C`: `tools/pipeline-canary.sh`, `tools/install-read-trace.sh` and
  `tools/install-pre-pr-gate.sh` all source it (the source-time unset alone protects
  `pipeline-canary.sh`'s own fixture-creation calls; the other two also call its
  `keel_repo_arg_guard` as their `<repo>` argument's validity gate). `tools/install-secret-guard.sh`
  inlines the same unset instead, at the top of the file, since it must stay copy-standalone with no
  `tools/lib/` dependency (its own tests run scratch copies that carry no `tools/lib/`). New
  `tests/test_repo_arg_guard_lib.sh`; `tests/test_secret_guard.sh` gains a decoy-repo block proving
  the vendor write lands only in the named `<repo>` and a non-git `<repo>` is refused regardless of
  the environment.
...
- **`tools/secret-guard/secret-scan.sh --range` (the pre-push scan) shared dir #508's same-change
  ...
  and confirmed live, fixed for the LOCAL pre-push hook only:** the boundary union initially only
  credited an entry that existed OUTSIDE the whole pushed range — an entry that arrived INSIDE the
  range via a merge, even one that genuinely predated the secret it exempts on the branch it came
  from, was invisible to it. Concretely: `main` legitimately adds an allowlist entry in one commit
  and, in a LATER commit, the secret it exempts (each individually clean against its own push-time
  baseline, already pushed to `origin/main`); a feature branch then `git merge origin/main`s both
  commits in together and pushes — the merge brought the pair INSIDE the range rather than leaving
  the entry at a boundary, so the otherwise-legitimate push falsely BLOCKED. Fixed the same way
  `resolve_range_local` already excludes known-remote content for a brand-new ref's own first-push
  shape: the baseline resolution (never the separate scan-for-secrets range) now appends `--not
  --remotes`, so a commit already reachable from ANY remote-tracking ref resolves as a boundary
  commit carrying whatever the earlier entry commit already added — but **only when a new
  `SECRET_SCAN_LOCAL_PUSH=1` env var (set exclusively by `tools/secret-guard/pre-push`, never by
  `ci-scan.sh`) says this is the local hook.** A SECOND max-review round (same cross-model reviewer,
  a follow-up pass) caught that applying the exclusion unconditionally is unsafe for `ci-scan.sh`:
  reproduced live via a real bare+clone CI topology, a CI checkout runs AFTER the push already
  landed, so the range's own TIP is typically already reachable from a remote-tracking ref too —
  excluding it collapsed EVERY CI scan's baseline to nothing, fail-closing every pre-existing
  allowlist entry on every ordinary push, not just the merge case this was meant to fix.
  `range-lib.sh`'s own `resolve_range_ci` docstring already named exactly this hazard for the
  scan-range itself; the baseline fix repeated it in a different spot. The local hook is the one
  caller where the exclusion is actually safe: it runs BEFORE the push transfers, so nothing in its
  own range can yet be reachable from a remote-tracking ref by construction, regardless of which
  branch of the new `if` runs — mutation-proved live for both rounds (reverting the union entirely
  reddens the merge-before-push assertions; reverting the `SECRET_SCAN_LOCAL_PUSH` gate reddens a
  new CI-clone-topology regression test in `tests/test_ci_secret_scan.sh` where the pre-existing
  entry would otherwise be wrongly dropped). The shared same-change message ("ignoring an allowlist
...
`docs/release-history.md`'s v0.9.0 entry.
...
- **Five findings from the v0.9.0 release-candidate delta audit (v0.8.3..5ba4dcb), all fix-before-tag,
  ...
  induced/original keyword was matched anywhere later in the line instead of the token immediately
  after the label (a genuine `original` mark whose own descriptive tail happened to contain the word
  "induced" flipped to induced), and the loop had no fenced-code-block exclusion (a quoted example of
  the convention counted as a real finding) — all three fixed together, anchoring the label, reading
  only the immediate next token, and blanking fenced blocks first via the existing
...
- **dir #386 leg (b): the first `/groom` run (v0.9.0 planning), and the three G0 retro amendments it
  ...
  fresh-reviewer adjudication round — and its G0 phase landed its findings where the doc says they
  belong: in the procedure docs themselves. `docs/delta-audit.md` §10 now states the narrower form of
  ...
  release names on the first count), and a just-shipped release's open acceptance legs are obligations
  of the new slate, never re-tagged and never counted as build items. Each amendment is pinned by its
...
- **dir #387: read-trace fuses — a session's own Read calls become a checkable trace instead of an
  unverifiable claim, closing the class dir #371/#375/the 2026-09-03 groom's own ticket-bodies-unread
  failure all share.** `tools/read-trace.sh` (opt-in via `tools/install-read-trace.sh`, same wiring
  discipline as `tools/pre-pr-gate.sh`) logs a session's `docs/*`/`commands/*.md`/`BACKLOG.md` Reads
  and its Edit/Write/NotebookEdit calls via a silent PostToolUse hook — silence is pinned by test
  (a printing hook is a red test). Tier 1: `/wrap`'s report and `/polish`'s PR body gain a
  `docs read:` line, generated by a shell helper from the log, never hand-typed. Tier 2: a persistent
  cross-release store feeds `read-trace.sh aggregate`'s small per-doc table (last-read date, read
  count, surface changes since) — dir #386's `/groom` G0 input once it ships. Tier 3 ships as DATA
  ONLY (`tools/read-trace-map.tsv`, a surface -> required-doc map with its own binding test) — the
  enforcing deny-hook is deliberately not built until a felt recurrence, an explicit operator
  decision, not an omission. The wrap-fuse amendment closes `/wrap`'s own gap as the pipeline's only
  structurally-skippable step: a `SessionEnd` hook classifies a mutating session as wrapped or not
  (via a `wrap-done` marker `commands/wrap.md`'s persist step now stamps), and — since `SessionEnd`'s
  own output reaches nobody, confirmed live — a `SessionStart` hook banners any pending miss at the
  START of the NEXT session instead, then clears it; excludes read-only sessions (trivially, no
  mutating rows) and DELEGATION RUN workers (forbidden to wrap by their own brief) via a transcript
  grep. `tests/test_read_trace.sh`/`test_install_read_trace.sh` cover the lib, the hook's silence, the
  wrap-fuse classifier (including a same-second wrap-then-mutate tie the tool's own comment names),
  both exclusions, and the aggregate's pinned format fed a synthetic log. Named boundary: every hook
  subcommand is Claude-Code-specific; no equivalent ships for another harness yet.

- **dir #386: `/groom [next-version]` — the grooming-manager pattern, closing the manager family's
  loop** (`/design` → `/go` → `/manage-release` → `/delta-audit` → `/groom` → feeds `/design` again).
  `docs/grooming.md` compresses two practised runs — the 2026-08-29 grooming pass and the 2026-09-03
  release-plan groom — into ten named phases (G0-G9): a retro-first phase that reads the previous
  cycle's records and APPLIES 2-3 process amendments directly into `docs/release-audit.md`,
  `docs/delta-audit.md` and `docs/drydock.md` via their own self-revision clauses, rather than leaving
  them as a report that dies in chat; release rows built only from named, operator-stated pains, never
  invented themes; deriving every list and count from live heading tags rather than hand-writing them,
...
the standard non-interactive `install.sh` run creates the `keel-polish.md` alias on its own, and from
that moment no re-run in any mode refreshes a drifted `commands/polish.md` again — see the deny-message
entry below for the mechanism and the affected messages. Those messages now describe this root cause
accurately, but do not fix it (dir #323). Second, a real-file (non-symlink) `bin/keel` is left untouched
by `install.sh`, whose refusal message says "remove it to let Keel wire the CLI." Both `bin/keel`'s own
severed-checkout advice and `doctor.sh`'s W-CLI-UNWIRED warning instead tell the reader to re-run
`install.sh`, so the one correct remedy is reachable only by reading `install.sh`'s own refusal message
(dir #324).
...
- **`tests/test_release_history.sh`'s tag<->heading reconciliation gained a bounded
  ...
  (`v0.7.1` inside `v0.7.10`), and a shallow clone's boundary commit could be misread as "just
  introduced here", fabricating a distance of 0. Supersedes dir #297's parts (b)/(c) as originally
  worded. Keel-self-maintenance: `tests/` and `docs/release-history.md` govern keel's own release
  process and are never installed into an adopter project.
- **A step-5 review receipt now carries at most ONE add-on; the comma-separated add-on SET is gone**
  ...
  (dir #201), silent when an `--amend` orphaned the retired round's lineage stamp (dir #214), and
  spuriously firing against an already-shipped round, complete with a durable `review-addon-dropped`
  log event advising a review that never saw the commit (dir #226). Deleting the set parser and the
  warning retires **all five structurally** rather than paying three separate fixes in rare-path code.
  Removed: the shared comma splitter, the outcome-set normalizer, dir #161's warning and its call site,
  and the unlock arm's element walk, which becomes a single allowlist call. **Kept, deliberately and
  unchanged:** the whole refusal-fallback branch (dir #70's subagent, the MANDATORY reminder dialog and
  its gate leg, the hand-off subcommands, `<level>-operator-run`, `<level>-waived`, `skip`), both add-on
  literals `+operator-run` (dir #81) and `+second-opinion` (dir #141) with their exact label strings,
