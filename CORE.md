...
<!-- Rails only, no placeholders. Edit between the markers, then mirror the block into
     templates/CLAUDE.md (test_core_wrapper_sync.sh pins byte equality). The KEEL-GIT markers fence
     the code/git rails that `install.sh --link --no-git` trims for machines without git. -->
