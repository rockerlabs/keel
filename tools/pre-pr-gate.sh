        ...
        agent:*+*,*)
          ...
          case "$addon_pre" in
            ...
            *)   depth_deny_reason="review-addon-set-retired"
                 depth_deny_msg="Pre-PR gate: step 5's review outcome ('$review_outcome') carries a COMMA-JOINED add-on suffix, which is no longer a valid shape — the receipt now names AT MOST ONE add-on (dir #183), so the suffix does not validate and the outcome fails the depth cross-check against step 4's '$depth_level'. If you just pulled Keel, an older COPIED commands/polish.md still teaches the comma-separated set: re-run install.sh and CHECK AFTERWARDS that your own commands/polish.md actually changed — its drift prompt does not update by default, and a non-interactive run can install Keel's copy under a keel- prefix while leaving yours stale. Re-installing with --link avoids the skew entirely. Then re-run this receipt naming exactly ONE add-on (operator-run takes the slot when both applied) and name every mechanism that reviewed this commit in the PR body and the step-10 summary instead. Run /polish again." ;;
          esac
          ;;
      esac
