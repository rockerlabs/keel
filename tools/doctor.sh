...
if [ "$INSTALL_MODE" = 1 ]; then
  ...
  if [ -f "$repo_root/keel" ]; then
    ...
    if [ -L "$kl" ] && [ -e "$kl" ]; then
      ...
      warn W-CLI-UNWIRED "keel CLI not wired at $ihome/bin/keel — re-run install.sh$imode_flag$ihome_flag --force (backs up a real file there first), or add an alias by hand"
    fi
  fi
