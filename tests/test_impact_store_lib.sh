...
# --- _impact_resolve_top memoization: repeated calls with the SAME dir return the same value fast,
# and a DIFFERENT dir is resolved fresh (not stuck on the first dir's cached answer) -------------------
other_repo="$(new_repo)"
run bash -c ". '$lib'
  ...
  [ \"\$a\" = \"\$c\" ] && { echo 'MISMATCH c matches a (stale cache)'; exit 1; }
  echo ok"
check_contains "_impact_resolve_top's single-slot cache hits on a repeat dir and misses on a new one" "$OUT" "ok"
