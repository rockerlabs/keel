...
**The leak gate is `export.sh`'s own rail, and it applies to mode A only.** `secret-guard/secret-scan.sh`
scans every file `export.sh` embeds TWICE — once before a single chunk byte is written (the caller's
file list plus `--disclosure-ack`/`--vendor` text) and once more over the fully assembled packet dir,
right before the success line (catching content the first pass can't name by field, like `--known`'s
copied-in file or the `remote:`/`repo:` URL this script embeds itself) — no bypass, no `--force`, no
`--skip-scan`; a hit at either pass refuses the whole export (exit 3), deletes anything already
written, and prints only the offending path, never the matched content. Mode B's brief is hand-
assembled, never run through `export.sh`, so this scan is not automatic there — whoever composes the
brief runs `secret-guard/secret-scan.sh` by
