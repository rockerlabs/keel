...
| [`tools/install-pre-pr-gate.sh`](../tools/install-pre-pr-gate.sh) | Wires the `/polish` gate's 5 hooks into a project's `.claude/settings.json` (project scope, the default) or `--global` (every repo). Opt-in and separate from `install.sh` on purpose: a hook changes what a session can do without asking each time. Same never-clobber discipline as `install-secret-guard.sh`. |
